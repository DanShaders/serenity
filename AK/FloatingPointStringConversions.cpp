...
public:
    ...
    {
        storage_copy(get_storage_of(value), get_storage(words_in_u64));
    }
    ...
    u64 top_64_bits(bool& has_truncated_bits) const
    {
        ...
        for (size_t i = 0; i < m_used_length; ++i) {
            size_t word_start = i * word_size, word_end = word_start + word_size;
    ...
    void multiply_with_power_of_5(u32 exponent)
    {
        ...
        //        reintrepret_cast and ifs) convert u64 to NativeWord (if NativeWord == u32)
        ...
        for (size_t i = 0; i < power_of_5.size(); ++i) {
            if (exponent >> i & 1) {
                if (power_of_5[i].size() == 1) {
                    ...
                    auto copy = *this;
                    storage_baseline_mul(copy.get_storage(), power_of_5[i],
                        get_storage(m_used_length + power_of_5[i].size()), g_null_stack_buffer);
    ...
    void multiply_with_power_of_2(u32 exponent)
    {
        if (exponent) {
            ...
            storage_shift_left(storage, exponent, storage);
            trim_last_word_if_zero();
        }
    }
    ...
    {
        return static_cast<CompareResult>(storage_compare(get_storage(), other.get_storage(), false));
    }
...
private:
    ...
    AK::UnsignedStorageReadonlySpan get_storage() const
    {
        return { m_words.data(), m_used_length };
    }
    ...
    void multiply_with_small(u64 value)
    {
        if (value <= max_word) {
            ...
            auto copy = *this;
            storage_baseline_mul(copy.get_storage(), get_storage_of(value), get_storage(m_used_length + 2), g_null_stack_buffer);
            trim_last_word_if_zero();
        }
    }
    ...
    void add_small(u64 value)
    {
        ...
        if (storage_add<false>(initial_storage, get_storage_of(value), expanded_storage))
