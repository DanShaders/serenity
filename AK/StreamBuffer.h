...
public:
    ...
    requires(SameAs<InvokeResult<Func, Bytes &&>, Coroutine<ErrorOr<size_t>>>)
    ...
    template<typename Func>
    requires(SameAs<InvokeResult<Func, Bytes &&>, ErrorOr<size_t>>)
    ErrorOr<size_t> enqueue(size_t preferred_capacity_for_writing, Func&& func)
    {
        allocate_enough_space_for(preferred_capacity_for_writing);
        size_t nread = TRY(func(Bytes { m_data + m_peek_head, m_capacity - m_peek_head }));
        m_peek_head += nread;
        return nread;
    }
